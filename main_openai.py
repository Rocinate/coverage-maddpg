...
def get_trainers(env, obs_shape_n, action_shape_n, arglist):
    ...
    if arglist.restore:  # restore the model
        for i in range(env.n):
            actors_cur[i] = (torch.load(arglist.old_model_name +
                             'a_c_{}'.format(i))).to(arglist.device)
            critics_cur[i] = (torch.load(
                arglist.old_model_name + 'c_c_{}'.format(i))).to(arglist.device)
            actors_tar[i] = (torch.load(arglist.old_model_name +
                             'a_t_{}'.format(i))).to(arglist.device)
            critics_tar[i] = (torch.load(
                arglist.old_model_name + 'c_t_{}'.format(i))).to(arglist.device)
            optimizers_a[i] = optim.Adam(
                actors_cur[i].parameters(), arglist.lr_a)
            optimizers_c[i] = optim.Adam(
                critics_cur[i].parameters(), arglist.lr_c)
    else:
        for i in range(env.n):
            actors_cur[i] = openai_actor(
                obs_shape_n[i], action_shape_n[i], arglist).to(arglist.device)
            critics_cur[i] = openai_critic(sum(obs_shape_n), sum(
                action_shape_n), arglist).to(arglist.device)
            actors_tar[i] = openai_actor(
                obs_shape_n[i], action_shape_n[i], arglist).to(arglist.device)
            critics_tar[i] = openai_critic(sum(obs_shape_n), sum(
                action_shape_n), arglist).to(arglist.device)
            optimizers_a[i] = optim.Adam(
                actors_cur[i].parameters(), arglist.lr_a)
            optimizers_c[i] = optim.Adam(
    ...
    actors_tar = update_train_tar(actors_cur, actors_tar, 1.0)
    # update the target par using the cur
    critics_tar = update_train_tar(critics_cur, critics_tar, 1.0)
...
def agents_train(arglist, game_step, update_cnt, memory, obs_size, action_size, actors_cur, actors_tar, critics_cur,
    ...
    if game_step > arglist.learning_start_step and \
        ...
        for agent_idx, (actor_c, actor_t, critic_c, critic_t, opt_a, opt_c) in \
            ...
            # set the data to gpu
            rew = torch.tensor(_rew_n, dtype=torch.float,
                               device=arglist.device)
            done_n = torch.tensor(
                ~_done_n, dtype=torch.float, device=arglist.device)
            action_n = torch.from_numpy(_action_n).to(
                arglist.device, torch.float)
            obs_n = torch.from_numpy(_obs_n).to(arglist.device, torch.float)
            obs_n_t = torch.from_numpy(_obs_n_t).to(
                arglist.device, torch.float)
            # cal the loss
            ...
            q = critic_c(obs_n, action_n).reshape(-1)
            # q value in next state
            q_t = critic_t(obs_n_t, action_tar).reshape(-1)
            ...
            nn.utils.clip_grad_norm_(
                critic_c.parameters(), arglist.max_grad_norm)
            opt_c.step()
            ...
            nn.utils.clip_grad_norm_(
                actor_c.parameters(), arglist.max_grad_norm)
            opt_a.step()
...
def train(arglist):
    ...
    for episode_gone in range(arglist.max_episode):
        ...
        if game_step > 1 and episode_gone % 500 == 0:
            ...
            coverage = round(np.mean(coverage_h[-500:-1]), 3)
            print(f'=Training: steps:{game_step} episode:{episode_gone} reward:{mean_ep_r} step:{mean_step} connected:{mean_connected_rate} collision:{mean_collision_rate} time:{int(end_time - start_time)} cov: {coverage}')
        ...
        for episode_cnt in range(arglist.max_episode_len):
            ...
            if not any([info_n[i]['collision'] for i in range(env.n)]) and all(
                ...
                memory.add(obs_n, np.concatenate(
                    action_n), rew_n, obs_n_t, done_n)
            ...
            is_collision = is_collision or any(
                [info_n[i]['collision'] for i in range(env.n)])
            is_connected = is_connected and all(
                [info_n[i]['connected'] for i in range(env.n)])
            coverage = info_n[-1]['coverage']
        ...
        if episode_gone > 0 and (episode_gone + 1) % arglist.fre_save_model == 0:
            ...
            print('=time:{} episode:{} step:{}        save'.format(
                time_now, episode_gone + 1, game_step))
            ...
            for agent_idx, (a_c, a_t, c_c, c_t) in \
                ...
                torch.save(a_c, os.path.join(
                    model_file_dir, 'a_c_{}.pt'.format(agent_idx)))
                torch.save(a_t, os.path.join(
                    model_file_dir, 'a_t_{}.pt'.format(agent_idx)))
                torch.save(c_c, os.path.join(
                    model_file_dir, 'c_c_{}.pt'.format(agent_idx)))
                torch.save(c_t, os.path.join(
                    model_file_dir, 'c_t_{}.pt'.format(agent_idx)))
    # save the curves
